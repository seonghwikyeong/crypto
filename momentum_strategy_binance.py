...
def set_leverage(symbol, leverage):
    """레버리지 설정"""
    try:
        response = client.futures_change_leverage(symbol=symbol.upper(), leverage=leverage, timestamp=get_current_timestamp(), recvWindow=5000)
...
def on_message(ws, message):
    ...
    if "k" in data:  # Kline 데이터
        ...
        if kline["x"]:  # 캔들이 종료되었는지 확인
            ...
            position_value = account_balance * 0.4 * (1/std)/(1/std + 1/btc_std) * leverage
